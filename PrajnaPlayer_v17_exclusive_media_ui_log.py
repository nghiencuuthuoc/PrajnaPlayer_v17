...
import datetime
# Media-key debug log
MEDIAKEY_LOG_FILE = Path('./log/mediakey.log')
...
# ----- Windows global media-key hook (AVRCP -> Media Keys) -----
class _WinMediaKeyHook:
    """Global low-level keyboard hook to catch Media Keys on Windows.

    Captures: Play/Pause, Next, Previous, Volume Up, Volume Down (and optionally Mute).
    Bluetooth earbuds (AVRCP) commonly surface as these keys on Windows.
    """
    WH_KEYBOARD_LL = 13
    WM_KEYDOWN = 0x0100
    ...
    def __init__(self, on_key, should_consume=None):
        self.on_key = on_key  # callback(vk_code:int)
        self.should_consume = should_consume  # callable(vk_code:int)->bool
        self._hook = None
        self._thread = None
        self._tid = None
        self._running = False
        ...
        self.LowLevelProc = ctypes.WINFUNCTYPE(
            wintypes.LRESULT, wintypes.INT, wintypes.WPARAM, wintypes.LPARAM
        )
        self._proc = self.LowLevelProc(self._hook_proc)
    ...
    def stop(self):
        ...
        try:
            if self._tid:
                self.user32.PostThreadMessageW(self._tid, self.WM_QUIT, 0, 0)
        except Exception:
            pass

    def _message_loop(self):
        self._tid = self.kernel32.GetCurrentThreadId()
    ...
    def _hook_proc(self, nCode, wParam, lParam):
        try:
            ...
            pass
        # Decide whether to swallow the key (exclusive control)
        try:
            if 'vk' in locals() and self.should_consume and self.should_consume(vk):
                return 1  # swallow
        except Exception:
            pass

        # Pass through so other apps / OS can also see it
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        ...
        self.exclusive_media_keys = tk.BooleanVar(value=True)
        ...
        # Media keys (Bluetooth headset AVRCP -> Windows media keys)
        self._media_hook = None
        self._start_media_key_hook()
    ...
    def _start_media_key_hook(self):
        """Enable global media-key hook on Windows to support Bluetooth headset controls."""
        if os.name != "nt":
            return
        try:
            self._media_hook = _WinMediaKeyHook(self._on_media_key, self._should_consume_media_key)
            self._media_hook.start()
        except Exception:
            self._media_hook = None
    def _log_mediakey(self, ts: str, action: str, vk: int):
        try:
            MEDIAKEY_LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
            with MEDIAKEY_LOG_FILE.open('a', encoding='utf-8') as f:
                f.write(f"{ts}\t{action}\t0x{vk:02X}\n")
        except Exception:
            pass

    def _on_media_key(self, vk: int):
        """Handle Windows media keys and map them to existing PrajnaPlayer actions."""
        try:
            # Marshal back to Tk main thread (hook runs in its own thread)
            if vk == _WinMediaKeyHook.VK_MEDIA_PLAY_PAUSE:
                self.root.after(0, self.toggle_play)
            elif vk == _WinMediaKeyHook.VK_MEDIA_NEXT_TRACK:
                self.root.after(0, self.next)
            elif vk == _WinMediaKeyHook.VK_MEDIA_PREV_TRACK:
                self.root.after(0, self.prev)
            elif vk == _WinMediaKeyHook.VK_VOLUME_UP:
                self.root.after(0, lambda: self._bump_volume(+5))
            elif vk == _WinMediaKeyHook.VK_VOLUME_DOWN:
                self.root.after(0, lambda: self._bump_volume(-5))
            # Mute is optional; keep Windows default behavior unless you implement app mute
            # elif vk == _WinMediaKeyHook.VK_VOLUME_MUTE:
    ...
    def _build_bottom_filters_bar(self):
        ...
        # Exclusive media keys (swallow when PrajnaPlayer is foreground)
        chk2 = tk.Checkbutton(right, text='Exclusive media keys', bg=BG, variable=self.exclusive_media_keys, onvalue=True, offvalue=False)
        chk2.pack(side=tk.LEFT, padx=(12,0))
    ...
    def _build_progress_row(self):
        ...
        self.media_status_lbl = ttk.Label(bar, textvariable=self.media_status_var)
        self.media_status_lbl.pack(side=tk.RIGHT)
    ...
    def _on_close(self):
        ...
        try:
            ...
            pass
        # Stop global media-key hook (if enabled)
        try:
            if getattr(self, "_media_hook", None):
                self._media_hook.stop()
        except Exception:
            pass
        self.root.destroy()

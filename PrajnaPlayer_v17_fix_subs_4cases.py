...
# ----- Subtitle language heuristics -----
# Vietnamese diacritics (covers most Vietnamese-specific codepoints)
_VI_DIACRITIC_RE = re.compile(r"[Đđ\u0102\u0103\u00C2\u00E2\u00CA\u00EA\u00D4\u00F4\u01A0\u01A1\u01AF\u01B0\u1EA0-\u1EF9]")

def _guess_lang_from_sub_content(path: Path, max_bytes: int = 80000) -> str:
    """Best-effort language guess for unlabeled subtitle files (no .en/.vi).

    Returns: 'vi' if Vietnamese diacritics are detected; otherwise 'en'.
    """
    try:
    ...
    # Strip timestamps / indices and sample a small window
    lines = []
    ...
    sample = " ".join(lines)
    return "vi" if _VI_DIACRITIC_RE.search(sample) else "en"
...
class PrajnaPlayerApp:
    ...
    def _build_sub_index(self, root: Path):
        """Build a subtitle index for the opened root folder.

        Index supports 4 cases:
        - <stem>.en.srt/.vtt
        - <stem>.vi.srt/.vtt
        - both .en and .vi
        - unlabeled: <stem>.srt/.vtt

        For unlabeled subtitles we store them under lang='und' and resolve to EN/VI
        on-demand via _guess_lang_from_sub_content().
        """
        idx: Dict[str, Dict[str, Path]] = {}

        def _prefer(existing: Path, new: Path) -> Path:
            # prefer .vtt over .srt when both exist for the same slot
            if existing and existing.suffix.lower() == ".srt" and new.suffix.lower() == ".vtt":
                return new
        ...
        for p in root.rglob("*"):
            ...
            strict_base = _suffix2_base(p)
            if strict_base:
                base = strict_base
                lang = Path(p.name).suffixes[-2].lower().lstrip(".")  # en/vi
            else:
            ...
            if lang in d:
                d[lang] = _prefer(d[lang], p)
            else:
                d[lang] = p

        self.sub_index = idx

    def _resolve_en_vi_from_map(self, d: Optional[Dict[str, Path]]) -> Tuple[Optional[Path], Optional[Path]]:
        if not d:
            return None, None
        en = d.get("en")
        vi = d.get("vi")
        ...
        if und and (en is None or vi is None):
            s_lang = _guess_lang_from_sub_content(und)
            if en is None and vi is None:
                ...
                if s_lang == "vi" and vi is None:
                    vi = und
                elif s_lang == "en" and en is None:
                    en = und

        return en, vi

    def _index_find_exact(self, base: str) -> Tuple[Optional[Path], Optional[Path]]:
        return self._resolve_en_vi_from_map(self.sub_index.get(base))
    ...
    def load_subtitle_manual(self):
        ...
        if base_str:
            ...
            if lang == "vi":
                ...
                # Unlabeled subtitle: guess language from content
                guessed = _guess_lang_from_sub_content(sel)
                expect = "vi" if guessed == "vi" else "en"
                en_candidate, vi_candidate = self._auto_pair_for(sel, expect=expect)
    ...
    def _auto_load_subtitle_for(self, audio_path: Path):
        ...
        # (1) Exact match in the same folder
        en_path, vi_path = self._exact_pair_for_audio(audio_path)
        single = self._exact_single_sub_for_audio(audio_path)

        if en_path or vi_path or single:
            en, vi = en_path, vi_path
            if single:
                s_lang = _guess_lang_from_sub_content(single)
                # If only single exists, map it to the detected track
                if en is None and vi is None:
                    if s_lang == "vi":
                        vi = single
                    else:
                    ...
                    # If one track is missing, try to fill it with the single subtitle
                    if s_lang == "vi" and vi is None:
                        vi = single
                    elif s_lang == "en" and en is None:
                        en = single
            self._load_dual_subtitles(en, vi)
            return

        # (2) Strict pairs in the same folder (both EN/VI present)
        pairs = _pair_subtitles_in_folder(folder)
        if pairs:
        ...
        # (3) Sub-index exact (whole opened root)
        ...
        # (5) Fallback fuzzy (same folder, supports .en/.vi and unlabeled)
    ...
    def _fuzzy_pick_for_audio(self, audio_path: Path) -> Tuple[Optional[Path], Optional[Path]]:
        ...
        Supports:
        - .en/.vi labeled subtitles (e.g., <base>.en.srt)
        - unlabeled subtitles (e.g., <base>.srt)

        For unlabeled subtitles we resolve to EN/VI via _guess_lang_from_sub_content().
        """
        folder = audio_path.parent
        stem = audio_path.stem
        min_score = 35.0
